...
TEMPERATURES=(271 272 273 274 275 276 277 278)
...
for temp in "${TEMPERATURES[@]}"; do
    data_file="../500atm/${temp}K/run_melt_${temp}K_500atm_mW.dat"
    if [ -f ${data_file} ]; then
        echo "$temp input file found: Ready to copy $temp data" >> ${LOG_FILE}
        cp ${data_file} ${WORKING_DIR}/run_melt_${temp}K_500atm_mW.dat
    else
...
gnuplot -e "set terminal png; \
            ...
            plot for [i=271:278] 'run_melt_'.i.'K_500atm_mW.dat' using 1:7 title ''.i.'K'; "
...
gnuplot -e "set terminal png; \
            ...
            set key right top ; \
            set style data lines; \
            plot for [i=271:278] 'run_melt_'.i.'K_500atm_mW.dat' using 1:5 title ''.i.'K'" > ${WORKING_DIR}/500atm/Tmelt_TE_${TRIAL}.png
...
gnuplot -e "set terminal png; \
            ...
            set key right top ; \
            set style data lines; \
            plot for [i=271:278] 'run_melt_'.i.'K_500atm_mW.dat' using 1:8 title ''.i.'K'" > ${WORKING_DIR}/500atm/Tmelt_MSD_${TRIAL}.png
...
gnuplot -e "set terminal png; \
            ...
            set key right top ; \
            set style data lines; \
            plot for [i=271:278] 'run_melt_'.i.'K_500atm_mW.dat' using 1:4 title ''.i.'K'" > ${WORKING_DIR}/500atm/Tmelt_VOL_${TRIAL}.png


for temp in "${TEMPERATURES[@]}"; do
    if [ -f ${WORKING_DIR}/run_melt_${temp}K_500atm_mW.dat ]; then
        echo "$temp input file found: Ready to remove $temp data" >> ${LOG_FILE}
        rm ${WORKING_DIR}/run_melt_${temp}K_500atm_mW.dat
    else
